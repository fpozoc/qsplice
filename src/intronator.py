...
def main():
    ...
    raw = f'data/raw/{args.version}'
    interim = f'data/interim/{args.version}'
    processed = f'data/processed/{args.version}'

    if args.version.startswith('g'):
        path = source.Gencode(version=int(args.version[1:]), specie='human').download(outdir=raw, type='gff3')
    else:
        path = args.file
...
def generate_introns(inpath: str, intronsdir: str = '.') -> str:
    ...
    print(f'Introns has been generated in: {outpath}')
    return outpath
